...
type SetupUnit = { id: string; mode: 'existing' | 'new'; name: string; code: string; unit_type: 'ready' | 'manufactured'; quantity: number; cost_price: number; sale_price: number };
const emptyUnit = (): SetupUnit => ({ id: '', mode: 'new', name: '', code: '', unit_type: 'ready', quantity: 1, cost_price: 0, sale_price: 0 });
...
export function ProductSetupWizardPage() {
  ...
  const [categories, setCategories] = useState<Category[]>([]); const [existingUnits, setExistingUnits] = useState<InventoryUnit[]>([]);
  ...
  useEffect(() => { void (async () => { let cq = supabase.from('categories').select('*').order('name'); if (branchFilter) cq = cq.eq('branch_id', branchFilter); const [cats, us] = await Promise.all([cq, supabase.from('inventory_units').select('*').eq('is_active', true).order('name')]); setCategories((cats.data as Category[]) || []); setExistingUnits((us.data as InventoryUnit[]) || []) })(); }, [branchFilter]);
  ...
  return <DesignSurface testId="product-setup-wizard-page"><DesignPageHeader title={isAr ? 'إضافة منتج متكامل' : 'Complete Product Setup'} subtitle={isAr ? 'المنتج ← الوحدات ← مراجعة' : 'Product → Units → Review'} actions={<Button variant="outline" size="sm" onClick={() => navigate('/products')}><ChevronLeft className="w-4 h-4" />{isAr ? 'العودة للمنتجات' : 'Back to products'}</Button>} /><DesignPanel>
    <div className="flex items-center gap-2 mb-6 overflow-x-auto pb-1">{[['1', isAr ? 'المنتج' : 'Product'], ['2', isAr ? 'الوحدات' : 'Units'], ['3', isAr ? 'مراجعة' : 'Review']].map(([n, label]) => { const active = Number(n) === step; const done = Number(n) < step; return <div key={n} className={`flex items-center gap-2 px-3 py-2 rounded-xl text-sm whitespace-nowrap ${active ? 'bg-brand-600 text-white' : done ? 'bg-ui-success-soft text-ui-success' : 'bg-ui-page-alt text-ui-subtle'}`}><span className="w-6 h-6 rounded-full flex items-center justify-center bg-white/20">{done ? <Check className="w-4 h-4" /> : n}</span>{label}</div>; })}</div>
